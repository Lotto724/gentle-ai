...
**Execution mode.** Subagent mode (Claude, Cursor, Kimi, Kiro): each review-*/jd-* subagent runs its own sweep-budgeted pass and returns its own ledger rows; merge those subagent ledger rows into the persisted ledger and persist per the branch above.
